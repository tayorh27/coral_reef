...
class GChatMyPosts extends StatefulWidget {

  // final Function(bool hide) hideFloatingButton;
  // GChatTimelineScreen(this.hideFloatingButton);

  @override
  State<StatefulWidget> createState() => _GChatMyPosts();
}

class _GChatMyPosts extends State<GChatMyPosts> {
  ...
  getGChats() async {
    ...
    Query gChatQuery = FirebaseFirestore.instance.collection("gchats").where("user_uid", isEqualTo: user.uid).where("visibility", isEqualTo: "published").orderBy("timestamp", descending: true).limit(gchatLimit);
    if(selectedTopics.isNotEmpty) {
      gChatQuery = FirebaseFirestore.instance.collection("gchats").where("user_uid", isEqualTo: user.uid).where("visibility", isEqualTo: "published").where("topics", arrayContainsAny: selectedTopics).orderBy("timestamp", descending: true).limit(gchatLimit);
    }
                    ...
                    Container(
                      ...
                      child: DefaultButton(
                        ...
                        press: () {
                          ...
                          //         builder: (BuildContext context) => new CreateNewGChat(mChat: chats[index])));
  ...
  onDeleteClick(int index) async {
    ...
    final confirm = await new GeneralUtils().displayReturnedValueAlertDialog(context, "Attention", "Are you sure you want to delete this post?", confirmText: "YES");
    if(confirm) {
      await gServices.deleteGChat(chats[index].id);
      // setState(() {
